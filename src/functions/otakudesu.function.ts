...
type AppwriteResponse = ServerResponse & {
...
interface RouteMatch {
  method: string;
  path: string;
  handler: keyof typeof otakudesuController;
}
...
function parseUrl(req: AppwriteRequest) {
...
function createNativeResponse(res: ServerResponse): AppwriteResponse {
  ...
  appwriteRes.status = function (code: number) {
    this.statusCode = code;
    return this;
  };

  appwriteRes.json = function (payload: unknown) {
    if (!this.headersSent) {
...
function matchRoute(pathname: string, method: string) {
  ...
  for (const route of routes) {
    ...
    const routeParts = route.path.split("/").filter((segment): segment is string => Boolean(segment));
    const requestParts = normalizedPath.split("/").filter((segment): segment is string => Boolean(segment));
...
function createRequest(req: AppwriteRequest, params: Record<string, string>) {
  return {
    method: req.method,
    url: req.url,
    headers: req.headers || {},
...
export default async function (req: IncomingMessage & AppwriteRequest, res?: ServerResponse) {
  ...
  try {
    const handler = otakudesuController[match.route.handler] as any;
